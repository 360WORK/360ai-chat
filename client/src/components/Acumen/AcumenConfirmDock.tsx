...
function AcumenConfirmDock({ frame, submitting = false }: AcumenConfirmDockProps) {
  ...
  return (
    <div
      ...
      className="mb-3 rounded-xl border border-border-medium bg-surface-secondary p-3"
    >
      <p className="text-sm font-semibold text-text-primary">{frame.title}</p>
      {frame.summary ? (
        <p className="mt-1 whitespace-pre-line text-sm text-text-secondary">{frame.summary}</p>
      ) : null}
